...
void WiFiManager::get_devices(){
  Serial.println("in get devices");
  String res = "{\"swith\":[{\"id\":1,\"title\": \"Bedroom\",\"control_url\":\"http://192.168.1.61/api/switch_light_bedroom\",\"check\":\"http://192.168.1.61/api/gt_status_bedroom\"},{\"id\":2,\"title\": \"Kitchen\",\"control_url\":\"http://192.168.1.61/api/switch_light_kitchen\",\"check\":\"http://192.168.1.61/api/gt_status_kitchen\"},{\"id\":3,\"title\": \"Hall1\",\"control_url\":\"http://192.168.1.61/api/switch_light_hall1\",\"check\":\"http://192.168.1.61/api/gt_status_hall1\"},{\"id\":4,\"title\": \"Hall2\",\"control_url\":\"http://192.168.1.61/api/switch_light_hall2\",\"check\":\"http://192.168.1.61/api/gt_status_hall2\"}]}";
  // res.replace("192.168.1.61",deviceIP);
  responseHtml(res);
}

void WiFiManager::add_api(String url, std::function<void()> func){

}
void WiFiManager::preSetupConfigPortal() {
...
void WiFiManager::setupConfigPortal() {
  ...
  server->on("/api/get_devices",                  std::bind(&WiFiManager::get_devices, this));
...
void WiFiManager::responseHtml(String msg)
{
  stopCaptivePortal();
  DEBUG_WM(DEBUG_VERBOSE,F("<- HTTP close"));
  handleRequest();
  
  server->sendHeader(FPSTR(HTTP_HEAD_CL), String(msg.length()));
  server->send(200, FPSTR(HTTP_HEAD_CT), msg);
}
